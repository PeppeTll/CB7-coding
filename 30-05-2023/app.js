import { navLink, socialList } from './mok.js'
  ...
  dataAll.then((data) => {
    ...
    .then(() =>
      postList.map(post => {
        ...
        return post;
      }).forEach(post => createTweetEl(post))
    )
}
...
const createTweetEl = (obj) => {
  ...
  const userAvatar = createEl('img', 'tweet_avatar', null, tweetWrapperEl, { name: 'src', value: obj.userImg });
  const wrapTweetInfo = createEl('div', 'wrap_tweet_info', null, tweetWrapperEl);
  const userName = createEl('p', 'user_name', obj.userName, wrapTweetInfo);
  const nickName = createEl('p', 'user_name', `@${obj.nickName}`, wrapTweetInfo);
